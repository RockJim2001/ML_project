...
import numpy as np
import pandas as pd
from geatpy.algorithms.moeas.nsga2.moea_NSGA2_archive_templet import moea_NSGA2_archive_templet as MOPSO
...
"""
    参考文献:
        [1] Reyes-Sierra M, Coello C A C. Multi-objective particle swarm optimizers: 
        A survey of the state-of-the-art[J]. International journal of computational
         intelligence research, 2006, 2(3): 287-308.
...
if __name__ == '__main__':
    ...
    """================================算法参数设置============================="""
    myAlgorithm = MOPSO(problem, population)  # 实例化一个算法模板对象
    ...
    dir_path = os.path.join(ROOT_PATH, 'result_change', parent_dir, DATASET_NAME.split('.')[0], 'MOPSO')
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    df1.to_csv(os.path.join(dir_path, f'Variable_MOPSO_{ML_method}.csv'), header=None, index=None)
    df2.to_csv(os.path.join(dir_path, f'Objective_MOPSO_{ML_method}.csv'), header=None, index=None)
